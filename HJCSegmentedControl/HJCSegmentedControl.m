...
#import "HJCSegmentedControl.h"
#import "HJCSegment.h"
#import "NYSegmentIndicator.h"

@interface HJCSegmentedControl ()
...
- (CGRect)indicatorFrameForSegment:(HJCSegment *)segment;

@end

@implementation HJCSegmentedControl
...
- (instancetype)initWithItems:(NSArray *)items {
    ...
    if (self) {
        ...
        for (NSString *segmentTitle in items) {
            HJCSegment *segment = [[HJCSegment alloc] initWithTitle:segmentTitle];
...
- (void)initialize {
    ...
    _segmentIndicatorAnimationDuration = 0.08f;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    for (HJCSegment *segment in self.segments) {
...
- (void)layoutSubviews {
    ...
    for (int i = 0; i < [self.segments count]; i++) {
        HJCSegment *segment = self.segments[i];
...
- (void)insertSegmentWithTitle:(NSString *)title atIndex:(NSUInteger)index {
    ...
    HJCSegment *newSegment = [[HJCSegment alloc] initWithTitle:title];
...
- (void)removeSegmentAtIndex:(NSUInteger)index {
    ...
    HJCSegment *segment = self.segments[index];
...
- (void)moveSelectedSegmentIndicatorToSegmentAtIndex:(NSUInteger)index animated:(BOOL)animated {
    ...
    if (index != self.selectedSegmentIndex) {
        HJCSegment *previousSegment = self.segments[self.selectedSegmentIndex];
        previousSegment.titleLabel.font = self.titleFont;
        previousSegment.titleLabel.textColor = self.titleTextColor;
    }
    
    HJCSegment *selectedSegment = self.segments[index];
    ...
    if (animated) {
        [UIView animateWithDuration:self.segmentIndicatorAnimationDuration
                         ...
                         completion:^(BOOL finished) {
                             if ([_delegate respondsToSelector:@selector(segmentAction:)]) {
                                 [_delegate segmentAction:self];
                             }
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.selectedSegmentIndicator.bounds, [touch locationInView:self.selectedSegmentIndicator])&&_enableSlide) {
        return YES;
    } else {
        // Otherwise, find the segment that the user touched, and select it
        [self.segments enumerateObjectsUsingBlock:^(HJCSegment *segment, NSUInteger index, BOOL *stop) {
